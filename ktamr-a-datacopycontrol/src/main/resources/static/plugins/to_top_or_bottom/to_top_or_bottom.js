(function($) {
    $.fn.dwseeTopBottomMenu = function(options) {
        ...
        $("#top-bottom-menu").css("height", settings.boxsize + "px")
            .css("bottom", (settings.boxsize * 2) + 10 + "px")
            .css("line-height", settings.boxsize + "px")
            .css(settings.position, "10px");
        $("#top-bottom-menu #dstbm-top").css("height", settings.boxsize + "px")
        ...
        $("#top-bottom-menu #dstbm-bottom").css("height", settings.boxsize + "px")
            .css("width", settings.boxsize + "px")
            .css(settings.position, "0")
            .css("top", (settings.boxsize * 2) + 2 + "px")
            .css("background", "url('" + settings.bottomicon + "') center center no-repeat")                                            .css("background-color", settings.boxbackground);
        $("#top-bottom-menu #dstbm-menu").css("height", settings.boxsize + "px")
            .css("width", settings.boxsize + "px")
            .css(settings.position, "0")
        ...
        $("#dstbm-menu").click(function() {
            $("#dstbm-menu #dstbmmenu").show();
            $("#dstbm-menu").css("background", settings.boxbackground
                + " url('') center center no-repeat").css("cursor", "default");
            $(this).animate({
        ...
        $("#dstbm-menu").hover(function() {},
            function() {
                    ...
                    $("#dstbm-menu").css("background", settings.boxbackground
                        + " url('') center center no-repeat").css("cursor", "pointer");
                    $(this).animate({
                            width: settings.boxsize + "px"
                        },
                        function() {
                            $("#dstbm-menu #dstbmmenu").hide();
                            $("#dstbm-menu").css("background", settings.boxbackground
                                + " url('" + settings.menuicon + "') center center no-repeat")
                    })
            })
    }
})(jQuery);